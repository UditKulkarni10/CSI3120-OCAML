...
let rec count_connectives (f:form) : int = 
        (match f with
            ...
            |False -> !count
            |Prop x -> !count
            |And(x, y)-> incr count; count_connectives(x); 
               count_connectives(y);
            |Or(x,y)-> incr count; count_connectives(x);
               count_connectives(y); 
            |Imp(x, y) -> incr count; count_connectives(x);
               count_connectives(y);)
...
let validate_env (e:env) : bool * (prop * bool) list = 
    ...
    let rec seen (u: prop * bool) (e:env) : bool * (prop * bool) list =
        let (a,b) = u in 
            match e with 
                |[] -> (false,[])
                |(x,y) :: t -> if x = a && y != b then (true, (x,y) :: []) else seen u t

    in let rec duplicate (e:env): (prop * bool) list = 
        match e with 
            |[]-> []
            |h::t -> match (seen h e) with 
                    |(false, []) -> duplicate(t)
                    |(false, _::_) -> []
                    |(true, x) -> (h::x) @ duplicate t
    
    in match duplicate e with    
        |[] -> (true, e)
        | h::t -> (false, duplicate e)
...
struct
  ...
  let enqueue (i:int) (s:t) : t = 
    i::s

  let peek (s:t) : int option = 
    match s with 
    | [] -> None
    | h:: _ -> Some h

  let dequeue (s:t) : t option = 
    let rec helper1 (l:t) (newL:t): t option = 
      match l with 
        |[]-> None
        |[x] -> Some newL
        |h::t -> helper1 t (newL@[h])
        in helper1 s []
end
...
struct
  ...
  let is_empty (s: 'a queue) =
    match s with
    | [] -> true
    | _::_ -> false

  let enqueue (i:'a) (s: 'a queue) : 'a queue = 
    i::s

  let peek (s: 'a queue) : 'a option = 
    match s with 
    | [] -> None
    | h::_ -> Some h

  let dequeue (s:'a queue) : 'a queue option = 
    let rec helper1 (l:'a queue) (newL:'a queue): 'a queue option = 
      match l with 
        |[]-> None
        |[x] -> Some newL
        |h::t -> helper1 t (newL@[h])
...
struct
  ...
  let enqueue (i:int) (s:t) : unit = 
    s := i::(!s)

  let dequeue (s:t) : int option = 
    let l = List.rev(!s) in 
      match l with 
      | [] -> None
      | h::t1 -> s := List.rev(t1); Some h
