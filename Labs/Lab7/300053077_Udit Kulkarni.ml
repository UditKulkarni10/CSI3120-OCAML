...
(* Give the sequence of steps for the function call (f1 11).  The
   ...
   call(f1 3)
   call(f1 1)
   raise odd exception in call f1 1
   pop

   handle exception f1 11
...
(* The following two versions of the "closest" function take an
   ...
   The second is sometimes more efficient than the first because if the leaf node matches the number 
   we are looking for the program terminates through the exception. This means it will not run through the rest of the program like
   it would in the first program. Since multiple recursive calls are made in both programs while traversing the tree to find the closest node, in the second one when 
   a leaf has the same value as the input integer, the error will be raised and so the other recursions will not need to complete for the solution to be presented. Hence 
...
exception LessThan0
let f (x:int) = 
  if x < 0 then raise LessThan0 
  else try x+1 with LessThan0 -> 0
